...
const showModal = (title, item = null)=>{
    console.log('Mostrando modal para:', title, 'con item:', item);
    currentItem = item;
    ...
    // Mostrar/ocultar campos según el tipo de formulario
    if (currentPage === 'products') {
        ...
        if (item) {
            console.log('Llenando formulario con datos:', item);
        ...
        if (item) {
            codeInput.value = item.name || '';
            emailInput.value = item.email || '';
        }
    }
    if (!formModal) formModal = new bootstrap.Modal(modal);
    formModal.show();
};
...
const renderProducts = ()=>{
    ...
    tbody.innerHTML = products.map((product)=>`
        <tr>
            ...
            <td>
                <button class="btn btn-sm btn-warning" onclick="showModal('Editar Producto', ${JSON.stringify(product)})">
                    <i class="bi bi-pencil"></i>
                </button>
                <button class="btn btn-sm btn-danger" onclick="deleteItem('products', ${product.id})">
...
const renderUsers = ()=>{
    ...
    tbody.innerHTML = users.map((user)=>`
        <tr>
            ...
            <td>${user.email}</td>
            <td>
                <button class="btn btn-sm btn-warning" onclick="showModal('Editar Usuario', ${JSON.stringify(user)})">
                    <i class="bi bi-pencil"></i>
                </button>
                <button class="btn btn-sm btn-danger" onclick="deleteItem('users', ${user.id})">
                    <i class="bi bi-trash"></i>
                </button>
            </td>
        </tr>
...
const deleteItem = async (type, id)=>{
    if (!confirm("\xbfEst\xe1 seguro de eliminar este elemento?")) return;
    try {
        ...
        showError(`Error al eliminar ${type === 'products' ? 'producto' : 'usuario'}`, error);
    }
};
// Navegación
...
window.deleteProduct = async (id)=>{
    if (confirm("\xbfEst\xe1s seguro de eliminar este producto?")) try {
        await axiosInstance.delete(`/api/products/${id}`);
        showAlert('Producto eliminado exitosamente');
        loadProducts();
    } catch (error) {
...
window.deleteUser = async (id)=>{
    if (confirm("\xbfEst\xe1s seguro de eliminar este usuario?")) try {
        await axiosInstance.delete(`/api/users/${id}`);
        showAlert('Usuario eliminado exitosamente');
        loadUsers();
    } catch (error) {
...
document.getElementById('saveButton').addEventListener('click', async ()=>{
    ...
    // Desactivar validación de email en productos
    if (currentPage === 'products') {
        email.removeAttribute('required');
        email.setCustomValidity('');
    } else email.setAttribute('required', '');
    console.log("Estado de validaci\xf3n de campos:", {
        code: code.validity.valid,
        description: description.validity.valid,
        price: price.validity.valid,
        stock: stock.validity.valid,
        category: category.validity.valid,
        email: email.validity.valid
    });
    if (!form.checkValidity()) {
        console.log("Formulario inv\xe1lido, mostrando validaciones");
        form.classList.add('was-validated');
        // Mostrar mensajes de error específicos solo para los campos relevantes
        if (currentPage === 'products') {
            if (!code.validity.valid) console.log("Error en c\xf3digo:", code.validationMessage);
            if (!description.validity.valid) console.log("Error en descripci\xf3n:", description.validationMessage);
            if (!price.validity.valid) console.log('Error en precio:', price.validationMessage);
            if (!stock.validity.valid) console.log('Error en stock:', stock.validationMessage);
            if (!category.validity.valid) console.log("Error en categor\xeda:", category.validationMessage);
        } else {
            if (!code.validity.valid) console.log('Error en nombre:', code.validationMessage);
            if (!email.validity.valid) console.log('Error en email:', email.validationMessage);
        }
        return;
    }
    try {
        if (currentPage === 'products') {
            ...
            console.log("\xbfEs actualizaci\xf3n?:", !!currentItem);
            console.log('ID del producto a actualizar:', currentItem?.id);
            let response;
            if (currentItem) {
                const url = `/api/products/${currentItem.id}`;
                console.log('Intentando actualizar producto en URL:', url);
                response = await axiosInstance.put(url, productData);
                console.log("Respuesta de actualizaci\xf3n:", response.data);
                showAlert('Producto actualizado exitosamente');
            } else {
                const url = '/api/products';
                console.log('Intentando crear producto en URL:', url);
                response = await axiosInstance.post(url, productData);
                console.log("Respuesta de creaci\xf3n:", response.data);
                showAlert('Producto creado exitosamente');
            }
            console.log("Operaci\xf3n completada, cerrando modal...");
            if (formModal) {
                formModal.hide();
                currentItem = null;
            ...
            console.log("\xbfEs actualizaci\xf3n?:", !!currentItem);
            console.log('ID del usuario a actualizar:', currentItem?.id);
            let response;
            if (currentItem) {
                const url = `/api/users/${currentItem.id}`;
                console.log('Intentando actualizar usuario en URL:', url);
                response = await axiosInstance.put(url, userData);
                console.log("Respuesta de actualizaci\xf3n:", response.data);
                showAlert('Usuario actualizado exitosamente');
            } else {
                const url = '/api/users';
                console.log('Intentando crear usuario en URL:', url);
                response = await axiosInstance.post(url, userData);
                console.log("Respuesta de creaci\xf3n:", response.data);
        ...
        showError('Error al guardar los datos', error);
    }
});
// Función para mostrar sección
const showSection = (section)=>{
    console.log("Cambiando a secci\xf3n:", section);
    currentPage = section;
    ...
    if (section === 'products') {
        ...
        if (productsSection) {
            ...
            loadProducts();
        } else console.error("No se encontr\xf3 la secci\xf3n de productos");
    } else if (section === 'users') {
        const usersSection = document.getElementById('usersSection');
        if (usersSection) {
            usersSection.style.display = 'block';
            loadUsers();
        } else console.error("No se encontr\xf3 la secci\xf3n de usuarios");
    }
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    if (!productsSection || !usersSection) {
        console.error('No se encontraron las secciones principales');
        return;
    }
    ...
    if (modalElement) formModal = new bootstrap.Modal(modalElement);
    else console.error("No se encontr\xf3 el modal");
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}]},["i4syP","9f5IL"], "9f5IL", "parcelRequire0671", {})

//# sourceMappingURL=proyecto-milimothitas.3052eed7.js.map
